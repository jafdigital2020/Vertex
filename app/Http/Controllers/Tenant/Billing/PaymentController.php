...
class PaymentController extends Controller
{
    ...
    public function initiatePayment(Request $request, $invoiceId)
    {
        ...
        try {
            ...
            $cleanMessage = "Failed to create payment request. Please try again later.";

            $this->logPaymentError(
                '[ERROR_CREATING_PAYMENT_REQUEST]',
                $cleanMessage,
                $request,
                $startTime
            );
            ...
            $cleanMessage = "Payment initiation failed,=. Please try again later.";
    ...
    public function checkStatus(Request $request, $transactionId)
    {
        ...
        try {
            ...
            $cleanMessage = "Failed to check payment status. Please try again later.";

            $this->logPaymentError(
                '[ERROR_CHECKING_PAYMENT_STATUS]',
                $cleanMessage,
                $request,
                $startTime
            );
    ...
    public function retryFailedSyncs(Request $request)
    {
        ...
        try {
            ...
            $cleanMessage = "Central admin sync retry failed. Please try again later.";
     ...
     */
    public function syncTransactionToCentralAdmin(Request $request, $transactionId)
    {
        $startTime = microtime(true);
        $authUser = $this->authUser();
        try {
            ...
            $cleanMessage = "Manual central admin sync failed Please try again later." . $e->getMessage();

            $this->logPaymentError(
                '[ERROR_MANUAL_CENTRAL_ADMIN_SYNC_TRANSACTION]',
                $cleanMessage,
                $request,
                $startTime
            );

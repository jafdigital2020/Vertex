...
class LeaveSettingsController extends Controller
{
   ...
   private function logLeaveSettingsError(
        string $errorType,
        string $message,
        Request $request,
    ...
    public function leaveSettingsCreate(Request $request)
    {
        ...
        try {
            ...
            $cleanMessage = "Failed to save leave settings. Please try again later.";

            $this->logLeaveSettingsError(
                '[ERROR_SAVING_LEAVE_SETTINGS]',
                $cleanMessage,
                $request,
                $startTime
            );

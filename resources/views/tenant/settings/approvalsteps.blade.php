...
@push('scripts')
    {{-- <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Renders one existing step into the accordion form
            ...
            // Populate a <select> with branchUsers array
            ...
            function addStep() {
                ...
                stepsAccordion.insertAdjacentHTML('beforeend', html);
                const sel = stepsAccordion.lastElementChild.querySelector('.user-select');
                populateUserSelect(sel);
            }

            // Render summary cards of existing steps
            function renderExistingSteps(steps) {
                if (!steps.length) {
                    existingContainer.innerHTML = '<p>No approval steps defined for this branch.</p>';
                    return;
                }
        ...
        <div class="card p-3" style="min-width:200px">
          ...
          ${userInfo}
            ...
            // When branch changes (or Global selected), fetch users & steps
            branchSelect.addEventListener('change', async () => {
                const branchId = branchSelect.value; // "" means Global (NULL in DB)
                ...
                try {
                    // 1) Fetch users (global or branch-specific)
                    let res = await fetch(
                    ...
                    if (!res.ok) throw new Error(`Users fetch status ${res.status}`);
                    ({
                        users: branchUsers
                    } = await res.json());

                    // 2) Fetch steps (global + any branch overrides)
                    res = await fetch(
                    ...
                    if (!res.ok) throw new Error(`Steps fetch status ${res.status}`);
                    const {
                        steps
                    } = await res.json();
            ...
            // Form submission: upsert global or branch-specific
            form.addEventListener('submit', async e => {
                e.preventDefault();
                const branchId = branchSelect.value || null; // empty→ null

                // gather steps
                const steps = Array.from(stepsAccordion.querySelectorAll('.accordion-item')).map(
                item => {
                    const lvl = +item.dataset.level;
                    const kind = item.querySelector('.approver-kind').value;
                    const uid = kind === 'user' ?
                ...
                try {
                    ...
                    if (!res2.ok) {
                        ...
                        throw new Error(err.message || 'Error saving steps');
                    }
                    const json = await res2.json();
                    toastr.success(json.message || 'Approval steps saved successfully!');
            ...
            // Remove a step card and re-index levels
            ...
            // Toggle the user-select dropdown on type change

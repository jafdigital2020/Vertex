...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const daysWrapper = document.getElementById('daysOfWeekWrapper');
            const customWrapper = document.getElementById('customDatesWrapper');
            const dayChecks = daysWrapper.querySelectorAll('input[name="days_of_week[]"]');



            let overrideAttempt = false;
            let skipRestAttempt = false;
            ...
            form.addEventListener('submit', async function(e) {
                ...
                const isRestDayChecked = document.getElementById('isRestDay').checked;
                const payload = {
                ...
                if (payload.type === 'recurring') {
                    payload.days_of_week = Array.from(dayChecks)
                        .filter(cb => cb.checked)
                        .map(cb => cb.value);
                    payload.custom_dates = [];
                } else if (payload.type === 'custom') {
                    payload.custom_dates = [].concat($('#customDates').val() || []);
                    console.log("Sending custom dates to server:", payload.custom_dates);
                    payload.days_of_week = [];
                }

                payload.override = overrideAttempt ? 1 : 0;
                payload.skip_rest_check = skipRestAttempt ? 1 : 0;

                try {
                    const response = await fetch('/api/shift-management/shift-assignment', {
                        method: 'POST',
                        headers: {
                    ...
                    if (response.ok) {
                        ...
                        if (data.requires_override) {
                            // prompt once for override vs skip
                            let choice = confirm(
                                data.message +
                                "\n\nPress OK to OVERRIDE the rest day, Cancel to SKIP it."
                            );

                            if (choice) {
                                // user chose OVERRIDE
                                overrideAttempt = true;
                                skipRestAttempt = false;
                                return form.dispatchEvent(new Event('submit'));
                            }

                            // user clicked “Cancel” on the first prompt → now ask SKIP explicitly
                            choice = confirm(
                                "Are you sure you want to SKIP the conflicting rest day instead?\n\n" +
                                "Press OK to SKIP, Cancel to CANCEL assignment."
                            );
                            if (choice) {
                                // user chose SKIP
                                overrideAttempt = false;
                                skipRestAttempt = true;
                                return form.dispatchEvent(new Event('submit'));
                            }

                            // user cancelled both dialogs
                            overrideAttempt = false;
                            skipRestAttempt = false;
                        ...
                        const doSkip = confirm("Do you want to SKIP the conflicting rest day instead?");
                    }
                } catch (err) {
                    console.error(err);

...
class TenUserSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $totalUsers = 10; // Changed from 90 to 10
        $createdCount = 0;
        $errorCount = 0;
        $overageInvoices = 0;
        ...
        try {
            for ($i = 1; $i <= $totalUsers; $i++) {
                try {
                    ...
                    // Progress indicator - only one status since all are within limit
                    $this->command->line("   [{$i}/{$totalUsers}] {$employeeId} - {$firstName} {$lastName} ✅ Within base limit");
                } catch (\Exception $e) {
                    $errorCount++;
